    ...
    public function buildings(int $plot): array
    {
        ...
        foreach ($data as $building) {
            ...
            $buildings[] = new Building([
                'number'           => (string) $building['byg007Bygningsnummer'],
            ...
            // Check for tekniske anlæg
            if (! ($building['id_lokalId'] ?? null)) {
                continue;
            }
            $q = http_build_query([
                'username' => config('plot-lookup.dk.datafordeler.username'),
                'password' => config('plot-lookup.dk.datafordeler.password'),
                'Format'   => 'JSON',
                'bygning'  => $building['id_lokalId'],
            ]);

            $u = "/BBR/BBRPublic/1/rest/tekniskanlaeg?{$q}";
            $techs = $client->get($u)->json();
            if (! $techs) {
                continue;
            }

            foreach ($techs as $tech) {
                $pnt = $tech['tek109Koordinat'] ?? null;
                if ($pnt) {
                    [$xx, $yy] = explode(' ', substr($pnt, 6, -1));
                }
                $buildings[] = new Building([
                    'number'           => 'T' . $tech['tek007Anlægsnummer'],
                    'usage'            => $this->translateCode('Klassifikation', $tech['tek020Klassifikation'] ?? null),
                    'built_at'         => $tech['tek024Etableringsår'] ?? '',
                    'roof_material'    => '',
                    'heating_device'   => '',
                    'area_residential' => $tech['tek039Effekt'] ?? null,
                    'x'                => (float) ($xx ?? 0),
                    'y'                => (float) ($yy ?? 0),
                    'icon'             => 'andet-anlaeg',
                ]);
